# Legacy Modernization Checker
## OWASP Top 10:2025 自動検証・対策生成ツール

Spring Boot / Spring Batch プロジェクト向けのセキュリティ自動検証テンプレートです。

## 目的
- **OWASP Top 10:2025** に準拠したセキュリティ検証を自動化
- 検出された脆弱性に対する **対策案を自動生成**
- 顧客・監査向けの **証跡（artifact）** を一括出力

### 主な機能
| 機能 | 説明 |
|------|------|
| **SAST** | Semgrep による静的解析（カスタムルール対応） |
| **SCA** | Dependency-Check による依存関係脆弱性検出 |
| **SBOM** | CycloneDX 形式の部品表生成 |
| **DAST** | ZAP による動的スキャン |
| **Container Scan** | Trivy によるイメージ/FS スキャン |
| **Secret Detection** | Gitleaks による機密情報検出 |
| **対策案自動生成** | 検出結果から `security-remediation.md` を生成 |
| **統合レポート** | 全結果を1つの HTML ダッシュボードに集約 |

---

## 1. プロジェクトの中身
- `.gitlab-ci.yml`：GitLab CI でフル検証
- `scripts/run_checks_local.sh`：ローカルでフル検証
- `tools/semgrep/.semgrep.yml`：自社 Semgrep ルール（Autofix 例を含む）
-   └/zap/automation.yaml`：ZAP Automation Framework の設定
-   └/remediate/generate_remediation.py`：対策案（security-remediation.md）自動生成
- `docs/migration-design.md`：移行設計のテンプレ（別途追記する運用）

---

## 2. 使い方（最短：持ち込みプロジェクトで結果を出す）

### 2.1 方法A：このテンプレを「既存プロジェクト」に追加（推奨）
1) 既存プロジェクトのルートに、以下を **コピー**（または Git Subtree/Submodule）  
   - `.gitlab-ci.yml`
   - `tools/`（semgrep, zap, remediate）
   - `scripts/`
2) 既存プロジェクトが Maven なら、そのまま動きます。Gradle でも SAST/DAST は動きます（SCA/SBOM は調整必要）。

### 2.2 方法B：このテンプレをベースに新規作成
- この repo をそのまま利用し、`app-*` を開発ベースにします。

#### モジュール構成
```
legacy-modernization-starter-v2/
├── pom.xml                  # 親POM（共通設定・依存管理）
├── app-core/                # コアモジュール：ドメインモデル、ビジネスロジック
├── app-security/            # セキュリティモジュール：認証・認可の共通設定
├── app-infra/               # インフラモジュール：DB/MQ/外部連携アダプタ（空）
├── app-web/                 # Webモジュール：REST API、Controller
├── app-batch/               # バッチモジュール：定期実行ジョブ
├── tools/                   # セキュリティ検証ツール設定
│   ├── semgrep/             # Semgrep カスタムルール
│   ├── zap/                 # ZAP Automation 設定
│   └── remediate/           # 対策案生成スクリプト
├── scripts/                 # ローカル実行スクリプト
├── docs/                    # 設計ドキュメント
└── reports/                 # 検証結果出力先（Git管理外）
```

| モジュール | 役割 |
|-----------|------|
| `app-core` | ドメインモデル、バリデーション、共通ユーティリティ |
| `app-security` | Spring Security 基線設定、認証・認可の共通コンポーネント |
| `app-infra` | Repository 実装、外部 API クライアント、MQ 連携（現在は空） |
| `app-web` | REST Controller、Web 固有の設定 |
| `app-batch` | Spring Batch ジョブ、スケジューラ設定 |

---

## 3. ローカル実行（すぐ結果を見る）
### 前提
- Java 21
- Maven 3.9+
- Docker（ZAP 用）
- Python 3.12+
- （任意）Semgrep：無ければ Docker で実行します（スクリプトは Docker 方式）
- **⚠️ ビルドが通ること**：`mvn package` が成功する状態で実行してください（SCA/SBOM はビルド成果物を解析します）

### 設定ファイル（重要）
スクリプトは `scripts/.env` から設定を読み込みます：

```bash
# 初回セットアップ（他のプロジェクトに適用する場合）
cp scripts/.env.sample scripts/.env
# エディタで .env を開いて、プロジェクトに合わせて編集
```

主な設定項目：

| 設定 | 説明 | 例 |
|-----|------|-----|
| `PROJECT_NAME` | レポートに表示するプロジェクト名 | `"My App"` |
| `PROJECT_TYPE` | プロジェクトタイプ | `"maven"` / `"npm"` / `"maven,npm"` |
| `BUILD_ROOT` | pom.xml/package.json の場所 | `"."` / `"backend"` |
| `NPM_ROOTS` | npm プロジェクトのパス（複数可） | `"frontend"` / `"frontend,packages/ui"` |
| `ENABLE_DAST` | DAST スキャンの有効化 | `"true"` / `"false"` |
| `APP_START_CMD` | アプリ起動コマンド | `"npm run start"` |
| `APP_PORT` | アプリのポート番号 | `"3000"` |

詳細は `scripts/.env.sample` を参照してください。

### 実行
Linux/macOS で実行する場合：
```bash
chmod +x scripts/run_checks_local.sh
./scripts/run_checks_local.sh
```

Windows で実行する場合：
```cmd
scripts\run_checks_local.bat
```

### 出力（結果）
実行結果は `reports/YYYY-MM-DD_HHMMSS/` フォルダに保存されます：

| ファイル | 説明 |
|---------|------|
| `semgrep.sarif` | SAST - Java（Semgrep 標準ルール）|
| `semgrep-custom.sarif` | SAST - Java（自社カスタムルール）|
| `semgrep-ts.sarif` | SAST - TypeScript/JS（Semgrep 標準ルール）|
| `semgrep-ts-custom.sarif` | SAST - TypeScript/JS（OWASP Top 10:2025 カスタムルール）|
| `dependency-check-report.json` | SCA（依存関係脆弱性）|
| `bom.json` | SBOM（CycloneDX 形式）|
| `zap-report.html` / `.json` | DAST（ZAP 動的スキャン）|
| `trivy-fs.json` | Container/FS スキャン（Trivy）|
| `trivy-image.json` | Docker イメージスキャン（Trivy）|
| `gitleaks.sarif` | Secret Detection（Gitleaks）|
| `security-remediation.md` | 対策案（Markdown）|
| **`security-report.html`** | **統合ダッシュボード（推奨）** |

> 💡 **統合HTMLレポート** は全ツールの結果を1つのダッシュボードに集約し、
> 重大度別サマリー・OWASP分類・前回比較が一目で確認できます。

---

## 4. GitLab CI 実行（プロジェクトを push すれば自動）
- `.gitlab-ci.yml` が同梱済みです。Push すると：
  - build/test
  - SAST（Semgrep）
  - SCA（Dependency-Check）
  - SBOM（CycloneDX）
  - DAST（ZAP）
  - 対策案生成（security-remediation.md）
が artifact として保存されます。

---

## 5. Autofix（自動修正）運用ルール（重要）
- **main ブランチでは自動修正しない**（差分が混ざると事故になるため）
- 修正専用ブランチ（例：`fix/security-*`）でのみ：
  - `semgrep --autofix` を許可
- 自動修正は「低リスク機械変換」のルールのみ（例：String の `==` → `equals()`）

---

## 6. 移行設計（延期防止）運用（必須）
- `docs/migration-design.md` を Phase A（判断）の成果として必ず埋める：
  - 一回切りの境界（One-Cut Boundary）
  - データ主権（SSOT）
  - 連携方式の優先順位（File/MQ/API）
  - Go/No-Go（合法に止まれるゲート）
これが無いと、後工程で必ず手戻りします。

---

## 7. よくある調整ポイント
- アプリの起動ポート：`dast_zap` は `localhost:8080` を前提
- 認証が必要な API：ZAP の context に認証設定（必要に応じて追加）
- 大規模アプリ：ZAP の探索範囲と時間を調整

---
## 8. 追加スキャン（Container / Secret）
このテンプレートは以下も同梱しています：
- Container Scanning：Trivy（Dockerイメージ/FS）
- Secret Detection：Gitleaks（SARIF）

**注意（既存プロジェクトへ適用する際）**
- Container Scanning（イメージスキャン）は `Dockerfile` がある場合に有効です。無い場合は FS スキャンのみ実行します。
- GitLab Runner が Docker-in-Docker を実行できる構成である必要があります（Container Scanning ジョブのみ）。

出力：
- `trivy-fs.json`（依存/OS層の検出含む）
- `trivy-image.json`（Dockerfile があり、イメージをビルドできる場合）
- `gitleaks.sarif`

---

## 9. 統合HTMLレポート（推奨）

### 概要
複数ツールの結果を1つのダッシュボードに統合し、以下を提供します：
- **エグゼクティブサマリー**：重大度別・ツール別の検出数
- **OWASP Top 10:2025 分類**：カテゴリ別の脆弱性分布
- **差分比較**：前回実行との比較（新規/修正済み/継続中）
- **詳細一覧**：フィルタリング・ソート可能なテーブル

### 実行履歴管理
```
reports/
├── 2025-01-15_143022/    # 実行日時フォルダ
│   ├── security-report.html
│   ├── security-remediation.md
│   ├── semgrep.sarif
│   └── ...
├── 2025-01-14_091530/    # 前回実行
└── latest -> 2025-01-15_143022/  # 最新へのシンボリックリンク
```

### 単独でHTMLレポートを生成
```bash
python tools/remediate/generate_html_report.py --input-dir reports/latest --output reports/latest/security-report.html
```
